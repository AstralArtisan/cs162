...
This is a CS 162 (Operating Systems) student repository. The primary work is in `pintos/`, which contains a Pintos teaching OS implementation. There are documents in `pintos/docs` for each project.

## Pintos Project

The active, modified code lives in `pintos/src/userprog/` (process control, syscalls, pthreads) and `pintos/src/threads/` (scheduler, synchronization).

### Build

```bash
cd pintos/src/userprog && make          # Build userprog project
cd pintos/src/threads  && make          # Build threads project
...
```bash
cd pintos/src/userprog && make check    # Run all userprog tests
cd pintos/src/threads  && make check    # Run all threads tests
cd pintos/src/userprog && make grade    # Show grade breakdown
```
...
Test sources are in `pintos/src/tests/userprog/` and `pintos/src/tests/threads/`.

## Architecture: Pintos Userprog

### Key Data Structures (`pintos/src/userprog/process.h`)
...
### Syscall Dispatch (`pintos/src/userprog/syscall.c`)
...
### Process Lifecycle (`pintos/src/userprog/process.c`)
