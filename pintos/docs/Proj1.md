...
- [Some Modification when doing Proj2](#some-modification-when-doing-proj2)
...
## Some Modification when doing Proj2

When doing proj2 multithreading, I suddenly find that something wrong in the structure we built before. In our previous implementation, we put all the new attributes we added for a user program into `struct thread`. Although we can pass all the tests, it's not good logically.

Recall that what we created: `child_list`, `child_process`, `next_fd`, `open_files` and `executable_file`. It's easy to see that they are all attributes for a whole process (maybe has multiple threads), but not actually for a single thread. Imagine a process with 2 threads. The 2 threads are working in different stack space, but they have the same files the process opened, and know which processes are their children. So, the patch we add is move these to `struct process`, making them as part of a process's `pcb`. With this idea, we need to change all the invocation before to `t->pcb->...`.

Also, we centralize `pcb` initialization to avoid partially initialized fields, add `NULL` guards so early or kernel-only threads don’t dereference a missing `pcb`. Below are the exact code changes.

**Key PCB fields moved to `process`:**
...
**Centralized PCB initialization and NULL guards**
...
**Exit path now safe when PCB is missing**
...
**Exception handling uses PCB child_process**
